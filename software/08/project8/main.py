...
def translate_single_file_vm_to_asm(inputFile, writer):
    ...
    while parser.hasMoreLines() :
        ...
        if commandType == 'C_ARITHMETIC': writer.writeArithmetic(arg1)

        if commandType in ['C_PUSH', 'C_POP']:writer.writePushPop(commandType, arg1, int(arg2))

        if commandType == 'C_LABEL': writer.writeLabel(arg1)

        if commandType == 'C_GOTO': writer.writeGoto(arg1)

        if commandType == 'C_IF': writer.writeIf(arg1)

        if commandType == 'C_FUNCTION': writer.writeFunction(arg1, arg2)

        if commandType == 'C_CALL': writer.writeCall(arg1, arg2)

        if commandType == 'C_RETURN': writer.writeReturn()

    parser.close()

if __name__ == '__main__':
    #dir_path = '/Users/daniel/Desktop/Nand2Tetris/projects/software/08/ProgramFlow/BasicLoop/BasicLoop.vm'
    #dir_path = '/Users/daniel/Desktop/Nand2Tetris/projects/software/08/ProgramFlow/FibonacciSeries/FibonacciSeries.vm'
